...
public class ZeromqSubscriberSingle
{
    public static void main (String [] args)
    {
        ...
        long messageCount = Integer.parseInt (args [1]);

        ZMQ.Context ctx = ZMQ.context (1);
        ZMQ.Socket s = ctx.socket (ZMQ.SUB);
        s.bind (bindTo);
        s.subscribe(new byte[0]);
        //s.subscribe("andy".getBytes());
        ...
        for (int i = 1; i != messageCount; i ++) {
            String subject = new String(s.recv (0));
            String contents = new String(s.recv (0));
            System.out.println("Subject:"+subject+" msg="+contents);
            //assert (data.length == messageSize);
        }
